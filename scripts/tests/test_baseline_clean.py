...
Each gate linter is run in-process against the REAL repo
...
that lint.yml enforces alongside the linters, so the full pre-push
...
# (module_name, entrypoint_attr) for each gate linter. Keep in sync with
# .github/workflows/lint.yml and scripts/hooks/pre-push.
LINTERS = [
    ("lint_blog", "main"),
    ("lint_vocab", "main"),
    ("lint_facts", "main"),
