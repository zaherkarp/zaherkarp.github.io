...
| `index.html` (hand-authored prose + marked regions) | the homepage itself | experience, projects, speaking, the hero (the education, service, and certifications SECTIONS were HTML-comment-disabled 2026-07-30 and deleted 2026-08-13; `cv.md` is their sole source now, the Gantt figure their only homepage surface, and `#education`/`#service` survive as bare anchors on it. See CLAUDE.md §Recognition alignment lint) |
...
| `lint_gantt.py` | every mark in the homepage Education+Service Gantt has a counterpart in `cv.md`'s Education/Service/Awards record, with each mark's year decoded from its x-coordinate. The gate direction flipped (figure ⊆ CV) when the `#education` and `#service` prose sections were deleted 2026-08-13 and the figure became the only visible surface. The two ids were restored the same day, as bare anchors on the figure's lead paragraph for external bookmarks; there is no prose section behind them |
