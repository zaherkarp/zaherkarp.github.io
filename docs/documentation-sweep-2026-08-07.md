...
Also out of scope, and left as recommendations in §6: adding test coverage.
...
Two genuine gaps surfaced that are code, not documentation, and were left for a
separate decision:

- **`lint_palette.py` has no test file.** Eleven of the twelve gate linters are
  exercised in `scripts/tests/`; palette is the exception.
- **The comment-disabled-section parsing has no test.** Both
  `test_lint_recognition.py` and `test_lint_gantt.py` build fixtures with *live*
  sections, so the property `CLAUDE.md` calls load-bearing, that the raw-text
  slice reads through HTML comments, is never exercised. If someone "fixed"
  those regexes to strip comments, both unit tests would still pass. The repo is
  currently protected only by accident: both linters guard against an empty
  parse and would fail loudly on the real tree. A fixture with a commented-out
  section would pin the property by name.
...
A fourth surfaced while reconciling `llms.txt` against the live page set:
**`cv.html` is missing from `sitemap.xml`.** `resume.html` is listed and
`cv.html` is not, because `build_blog.py` hardcodes the resume beside its
`SUBPAGES` list and the CV was never added to either. The CV is a real page,
linked from `#contact` and the site footer, so it is currently invisible to
crawlers that work from the sitemap. `lint_links.py` cannot catch this: it
verifies that every sitemap entry resolves to a file, not that every real page
appears in the sitemap, so the gate is green by construction. Fixing it is a
one-line change to `SUBPAGES` plus a regenerated `sitemap.xml`, which is a build
change rather than a documentation change and was left for a separate commit.

A fifth item is a judgement call rather than a gap: the Certifications block's
trailing rule sits inside the disabled region, so restoring that section per its
own instructions yields two adjacent rules. The comment's reasoning was
corrected in this pass; moving the rule would be a rendering change and was not
done.
