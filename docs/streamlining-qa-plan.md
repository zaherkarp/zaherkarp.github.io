...
| Extend `lighthouse.yml` | Also audit `/life-in-weeks/` and `/epidemic-simulation/`, and add their paths to the workflow triggers. | **DONE 2026-07-12.** Both subpage URLs added to the audit set and their `**` globs to the `pull_request` triggers. The coverage caught real a11y gaps and one config decision: (1) the epidemic simulator's three range sliders had visible label text but no programmatic label, fixed with `aria-labelledby`; (2) both subpages lacked a `<main>` landmark (`landmark-one-main`), added a layout-neutral `<main>` wrapper to each; (3) the epidemic page's in-prose links relied on color alone (`link-in-text-block`), given a persistent underline scoped to content paragraphs. All a11y fixes are invisible or on-brand and keep the accessibility `error` gate strict for both pages. (4) The epidemic page also trips `deprecations` + `unused-javascript` at error level from the **Pyodide + Plotly CDN runtimes** it fundamentally requires (the documented CDN-runtime exception, CLAUDE.md §Stack) — no markup change can fix third-party bundle size / deprecated-API use. Resolved by a per-URL `assertMatrix` in `.lighthouserc.json` (owner decision) that scopes only those two audits to `warn` for `/epidemic-simulation/`, keeping every other assertion (accessibility + SEO at `error`) strict for it and leaving all other URLs fully strict. Same spirit as the ~10 perf audits the config already turns off site-wide for architectural reasons; no currently-passing page's bar moves. |
