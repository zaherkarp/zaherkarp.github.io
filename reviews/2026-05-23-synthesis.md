...
Sparkline regeneration. Split off from Iteration 5a because it
touches `scripts/build_portfolio.py` and the CI workflow that
fires on changes to `scripts/`.
...
Production-system anchor still pending. Carries over from
Iteration 4's Tier 1 — author has not yet shared a number safe for
public surface.
