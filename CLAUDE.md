...
The 6 most recent non-draft posts in the homepage Writing section are
generated by `scripts/build_portfolio.py` between
`<!-- writing-list:start --> ... <!-- writing-list:end -->` markers,
sourced from `src/content/blog/*.md` frontmatter (publishDate, draft,
title, description). Do not hand-edit the entries between those markers,
the next CI run will overwrite them. To attach a margin note to a
homepage entry, add an optional `homepageMarginnote: "..."` field to
that post's frontmatter; the build wraps it in a ⊕ toggle next to the
title with id `mn-w-<slug>`.

The activity sparkline above the entries is generated by the same
script, between `<!-- activity-grid:start --> ... <!-- activity-grid:end -->`
...
desc then alphabetic). The "View all writing" link below the entries
is fixed prose (outside the markers) and points to `/blog/`.
...
The portfolio writing section shows the 6 most recent non-draft posts,
generated by `scripts/build_portfolio.py` from blog frontmatter. See
§Writing section update rule above for the marker contract.
